    ...
    public class ItemMonitorController : BaseController<ItemMonitoramento, ItemMonitoramentoViewModel, IItemMonitoramentoAppService>
    {
        ...
        public override HttpResponseMessage Query(SearchRequest request)
        {
            ...
            if ((result?.Itens.Any()).GetValueOrDefault())
            {
                ...
                IndicadoresMonitoramento resumo;

                var filterGestor = request.Filter.Rules.FirstOrDefault(x => x.Field == "IdGestorMonitor");
                if (filterGestor != null)
                {
                    var indicadoresExpression = QueryBuilder.BuildQueryExpression<IndicadoresMonitoramento>(filterGestor, false, null);
                    resumo = this._indicadoresService.Buscar(indicadoresExpression).Where(x => x.DataMonitoramento == dataMonitoramento && x.IdProgramaSaude == response.IdPrograma).FirstOrDefault();
                    resumo.QuantidadeAlertasItens = resumo.QuantidadeAlertasItensGestor;

                }
                else
                {
                    resumo = this._indicadoresService.Buscar(x => x.DataMonitoramento == dataMonitoramento && x.IdProgramaSaude == response.IdPrograma)
                        .GroupBy(x => new { x.DataMonitoramento, x.NomePrograma, x.QuantidadeItens, x.QuantidadeAlertasItens, x.TipoConsumoAutonomia, x.TipoConsumoSaldoZerado }, (g, itens) => new IndicadoresMonitoramento
                        {
                            JustificativasPendentes = itens.Sum(s => s.JustificativasPendentes),
                            QuantidadeAlertas = itens.Sum(s => s.QuantidadeAlertas),
                            DataMonitoramento = g.DataMonitoramento,
                            NomePrograma = g.NomePrograma,
                            QuantidadeItens = g.QuantidadeItens,
                            QuantidadeAlertasItens = g.QuantidadeAlertasItens,
                            TipoConsumoSaldoZerado = g.TipoConsumoSaldoZerado,
                            TipoConsumoAutonomia = g.TipoConsumoAutonomia
                        }).FirstOrDefault();
                }
