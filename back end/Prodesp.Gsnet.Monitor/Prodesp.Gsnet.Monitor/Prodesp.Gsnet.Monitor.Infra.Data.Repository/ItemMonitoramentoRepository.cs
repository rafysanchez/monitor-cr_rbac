...
namespace Prodesp.Gsnet.Monitor.Infra.Data.Repository
{
    public class ItemMonitoramentoRepository : EFRepository<ItemMonitoramento>, IItemMonitoramentoRepository
    {
        ...
        public IEnumerable<ItemMonitoramentoDTO> BuscarPorPrograma(int idPrograma, int? idGestor = null)
        {
            var query = idGestor.HasValue ?
                        this.QueryBase().Where(x => x.IdPrograma == idPrograma && x.IdGestorMonitor == idGestor) :
                        this.QueryBase().Where(x => x.IdPrograma == idPrograma);
            return query.ToList();
        }
