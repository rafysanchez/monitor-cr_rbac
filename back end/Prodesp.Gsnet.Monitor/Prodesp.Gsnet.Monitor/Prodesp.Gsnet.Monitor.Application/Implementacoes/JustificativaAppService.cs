...
namespace Prodesp.Gsnet.Monitor.Application.Implementacoes
{
    public class JustificativaAppService : AppService<Justificativa, IJustificativaService>, IJusticativaAppService
    {
        ...
        public ValidationResult JustificarPorPrograma(int idPrograma, int idJustificador, MotivoAcao motivo, MotivoAcao acao,int? idGestor)
        {
            ValidationResult result = new ValidationResult();
            try
            {
                this._service.JustificarPorPrograma(idPrograma, idJustificador, motivo, acao,idGestor);
            }
            catch (Exception e)
            {
