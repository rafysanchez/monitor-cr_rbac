...
export class ProdespMonitorModalPesquisaMedicamentoComponent extends ModalComponent<IModalPesquisaMedicamento> implements OnInit, IModalPesquisaMedicamento {
   ...
   * @param dialogService
   */
  constructor(dialogService: DialogService, private programaSaudeService: ProgramaSaudeService) {
    super(dialogService);
  }
  ...
  ngOnInit() {
    ...
    this.programaSaudeService.buscarTodos().map((response: Response) => {
      if (response && Array.isArray(response)) {
         response.map(item => {
            this.programasSaude.push( new ProgramaSaude(item.IdProgramaSaude,  item.NomePrograma));
         });
      }
    }).subscribe();
    this.situacoesSaldo = [
