import { ProdespMonitorModalAcaoBlocoComponent } from './../prodesp-monitor-modal-deletar/prodesp-monitor-modal-deletar.component';
...
export class ProdespMonitorJustificativaComponent implements OnInit {
  ...
  user: User;
  constructor(private appConfig: AppConfig, private userService: UserService, private dialogService: DialogService, private justificativaService: JustificativaService, private service: JustificativaService) {
        ...
        .map((response: Response) => {
          ...
          if (result.Message === '' || result.Message === null) {
            ...
            result.Data.forEach((item: any) => {
              item.FluxoJustificacao = new FluxoJustificacao(item.JustificadoFME, item.JustificadoCAF, item.JustificadoCMP, item.JustificadoCAFPublica);
  ...
  incluirFiltroFarmacia(rules: FilterRule[]): void {
    if (this.user.IdFarmacia) {
      let rule = new FilterRule('IdGestorMonitor', this.user.IdFarmacia.toString(), 'AND', 'equal', 'integer');
      rules.push(rule);
    }
  }
...
*/
    ...
    this.dialogService.addDialog(ProdespMonitorModalNovaJustificativaComponent, data).subscribe((novaJustificativa) => {
      if (novaJustificativa) {
        ...
        dataToPost.Data = {
          ...
          IdGestorMonitor : this.user.IdFarmacia
        };
  ...
  excluirJustificativas(): void {
    this.dialogService.addDialog(ProdespMonitorModalAcaoBlocoComponent, {
  ...
  abrirModalJustificativa(data: IModalNovaJustificativa): void {
    this.dialogService.addDialog(ProdespMonitorModalNovaJustificativaComponent, data).subscribe((novaJustificativa: Justificativa) => {
      if (novaJustificativa) {
        this.isLoading = true;
        let dataToPost: any = novaJustificativa;
        dataToPost.Itens = this.linhasSelecionadas.map((item: any) => {
