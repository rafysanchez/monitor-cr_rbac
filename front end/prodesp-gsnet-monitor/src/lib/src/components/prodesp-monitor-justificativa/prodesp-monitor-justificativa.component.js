...
var prodesp_monitor_modal_deletar_component_1 = require("./../prodesp-monitor-modal-deletar/prodesp-monitor-modal-deletar.component");
...
var ProdespMonitorJustificativaComponent = (function () {
    function ProdespMonitorJustificativaComponent(appConfig, userService, dialogService, justificativaService, service) {
        ...
        this.idJustificador = this.user.IdJustificador;
    }
            ...
            this.justificativaService.getHttpContext().post(this.appConfig.getConfig('host') + "api/itemmonitor/queryitens?TYPE=json", request)
                ...
                if (result.Message === '' || result.Message === null) {
                    ...
                    result.Data.forEach(function (item) {
                        item.FluxoJustificacao = new fluxo_justicacao_1.FluxoJustificacao(item.JustificadoFME, item.JustificadoCAF, item.JustificadoCMP, item.JustificadoCAFPublica);
    ...
    ProdespMonitorJustificativaComponent.prototype.incluirFiltroFarmacia = function (rules) {
        if (this.user.IdFarmacia) {
            var rule = new filter_rule_1.FilterRule('IdGestorMonitor', this.user.IdFarmacia.toString(), 'AND', 'equal', 'integer');
            rules.push(rule);
        }
    };
    ProdespMonitorJustificativaComponent.prototype.montarPesquisa = function () {
        ...
        this.incluirFiltroFarmacia(rules);
        if (rules.length) {
        ...
        this.dialogService.addDialog(nova_justificativa_component_1.ProdespMonitorModalNovaJustificativaComponent, data).subscribe(function (novaJustificativa) {
            if (novaJustificativa) {
                ...
                dataToPost.Data = {
                    ...
                    IdGestorMonitor: _this.user.IdFarmacia
                };
    ...
    ProdespMonitorJustificativaComponent.prototype.excluirJustificativas = function () {
        var _this = this;
        this.dialogService.addDialog(prodesp_monitor_modal_deletar_component_1.ProdespMonitorModalAcaoBlocoComponent, {
            title: 'Excluir Justificativas selecionadas',
            text: 'Tem certeza que deseja excluir as justificativas para os medicamentos selecionados? <br />Essa operação não poderá ser desfeita',
            closeButtonText: 'Fechar',
            confirmButtonText: 'Excluir',
            showConfirmButton: true,
            UsarJustificativaCAF: false,
            UsarJustificativaCAFPublica: false
        }).subscribe(function (data) {
            if (data) {
                _this.isLoading = true;
                var dataToPost = {};
                ...
                dataToPost.UsarJustificativaCAF = data.UsarJustificativaCAF;
                dataToPost.UsarJustificativaCAFPublica = data.UsarJustificativaCAFPublica;
            ...
            if (novaJustificativa) {
                ...
                dataToPost.Itens = _this.linhasSelecionadas.map(function (item) {
                    return {
                        ...
                        IdItemGsnet: item.data.IdItemGsnet,
                        IdGestorMonitor: item.data.IdGestorMonitor,
                        UsarJustificativaCAF: novaJustificativa.UsarJustificativaCAF,
                        UsarJustificativaCAFPublica: novaJustificativa.UsarJustificativaCAFPublica
                    };
                });
                dataToPost.IdJustificador = _this.idJustificador;
...
ProdespMonitorJustificativaComponent = __decorate([
    ...
    __metadata("design:paramtypes", [config_service_1.AppConfig, user_service_1.UserService, ng2_bootstrap_modal_1.DialogService, justificativa_service_1.JustificativaService, justificativa_service_1.JustificativaService])
